...
describe VCAP::Logging::Logger do
  ...
  describe '.define_log_levels' do
    it 'should raise an exception if supplied with non-log level map' do
      lambda { VCAP::Logging::Logger.define_log_levels('foo') }.should raise_error(ArgumentError)
    end
  ...
  describe '#log' do
    it 'should raise an exception if called with a non-array tag option' do
      lambda { @logger.log(0, "foo", :tags => 'foo') }.should raise_error(ArgumentError)
    end
  ...
  describe '#logf' do
    it 'should raise an exception if called with an invalid format string' do
      lambda { @logger.logf(:level0, [], []) }.should raise_error(ArgumentError)
    end

    it 'should raise an exception if called with invalid format args' do
        lambda { @logger.logf(:level0, "%s", 'foo') }.should raise_error(ArgumentError)
    end

    it 'should raise an exception if called with a non-array tag option' do
      lambda { @logger.logf(:level0, "foo", :tags => 'foo') }.should raise_error(ArgumentError)
    end
