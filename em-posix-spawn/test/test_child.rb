...
class ChildTest < Test::Unit::TestCase
  ...
  def test_add_listeners
    em do
      ...
      listeners = listeners.to_a.sort { |x,y| x.name <=> y.name }
  ...
  def test_listener_empty_streams_completed_process
    em do
      ...
      p.callback do
        ...
        # Event Machine times out if listeners are not called required number
        # of times.
        check = lambda do
          done if num_calls == 2
          EM.next_tick { check.call }
        end
  ...
  def test_listener_nonempty_streams_completed_process
    em do
      p = Child.new("echo -n test >& 1; echo -n test >& 2")
      p.callback do
        assert p.success?
  ...
  def test_listener_large_stdout
    ...
    em do
      p = Child.new("echo -n #{output_a}; sleep 0.1; echo -n #{output_b}")
      received_data = ''
      listeners = p.add_streams_listener do |listener, data|
        assert listener
        assert data
  ...
  def test_listener_nonempty_streams_active_process
    em do
      command = "echo -n A; sleep 0.01"
      command << "; echo -n B; sleep 0.01"
      command << "; echo -n C; sleep 0.01"
      p = Child.new(command)
      ...
      p.add_streams_listener do |listener_outer, data_outer|
        data[0] << data_outer
        closed[0] = true if listener_outer.closed?
        unless called
          EM.next_tick do
            p.add_streams_listener do |listener_inner, data_inner|
              data[1] << data_inner
              closed[1] = true if listener_inner.closed?
            end
          end
      ...
      p.callback do
        ...
        # Event Machine times out if both listeners are not closed.
        check = lambda do
          done if closed[0] && closed[1]
          EM.next_tick { check.call }
        end
