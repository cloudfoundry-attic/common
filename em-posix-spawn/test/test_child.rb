...
class ChildTest < Test::Unit::TestCase
  ...
  # Tests if a listener correctly receives incremental stream updates from a
  # process that has already finished execution, and has produced large output
  # in stdout.
  def test_listener_large_stdout
    output_chunk = "a" * 1024 * 32
    output = output_chunk * 2

    em {
      p = Child.new("echo -n #{output}")
      p.callback {
        assert p.success?
        ...
        EM.next_tick {
          ...
          num_sentinels = p.add_streams_listener { |update|
            if update.sentinel?
              ...
              assert_equal output_chunk, update.data
